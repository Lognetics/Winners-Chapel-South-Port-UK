    ...
    <Section tone="darker" className="grain">
      <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:gap-16">
        <Reveal direction="right" className="lg:sticky lg:top-28 lg:self-start">
          ...
          />
          <div className="mt-8 overflow-hidden rounded-3xl ring-1 ring-gold-500/20">
            <div className="relative aspect-[16/10]">
              ...
              <div className="absolute bottom-5 left-5">
                ...
                <p className="text-sm text-cream/70">Food bank beneficiaries served</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {csrPrograms.map((p, i) => (
            <Reveal key={p.title} delay={(i % 2) * 0.08} direction="left">
              <div className="card-lift flex h-full flex-col gap-3 rounded-2xl bg-navy-900/50 p-6 ring-1 ring-gold-500/10">
                ...
                <p className="flex-1 text-sm leading-relaxed text-cream/55">{p.desc}</p>
                <p className="text-xs font-semibold uppercase tracking-wider text-gold-400">{p.stat}</p>
              </div>
            </Reveal>
          ))}
        </div>
