  ...
  return (
    <Section id="services" tone="darker">
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {services.map((s, i) => (
          <Reveal key={s.name} delay={i * 0.06}>
            <div className="card-light group relative flex h-full flex-col overflow-hidden rounded-3xl p-7">
              <div className="pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full bg-gold-500/10 blur-2xl opacity-0 transition-opacity group-hover:opacity-100" />
              <div className="flex items-center justify-between">
                <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-gold-300/30 to-gold-500/20 text-gold-700 ring-1 ring-gold-500/25">
                  <Icon name={s.icon} className="h-6 w-6" />
                </span>
                <span className="rounded-full bg-navy-900/5 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-gold-700">
                  {s.tag}
                </span>
              </div>
              ...
              <div className="mt-6 flex items-center gap-4 border-t border-navy-900/10 pt-4">
                <button className="text-xs font-semibold uppercase tracking-wider text-gold-700 transition hover:text-gold-600">
                  + Add to Calendar
                </button>
              </div>
            </div>
      ...
      <Reveal className="mt-8">
        <div className="flex flex-col items-center gap-6 rounded-3xl bg-navy-900 p-6 sm:flex-row sm:justify-between sm:p-8">
