          ...
          {actions.map((a, i) => (
            <Reveal key={a.title} delay={i * 0.08}>
              <div className="card-light group flex h-full flex-col rounded-3xl p-8">
                ...
                <h3 className="mt-6 font-display text-2xl font-semibold text-navy-900">{a.title}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-navy-600/80">{a.desc}</p>
                <div className="mt-6">
          ...
          <div className="flex items-center px-6 py-16 sm:px-12 lg:py-24">
            <Reveal>
              <span className="inline-flex items-center gap-2.5 text-xs font-semibold uppercase tracking-[0.25em] text-gold-600">
                <span className="h-px w-6 bg-gold-500/70" /> Visit Us
              </span>
              ...
              <div className="mt-6 space-y-4 text-navy-700">
                <p className="flex items-start gap-3"><Icon name="map-pin" className="mt-0.5 h-5 w-5 text-gold-600" />{site.fullAddress}</p>
                <p className="flex items-center gap-3"><Icon name="clock" className="h-5 w-5 text-gold-600" />Sundays 8:00 AM &amp; 10:30 AM · Wednesdays 6:00 PM</p>
                <p className="flex items-center gap-3"><Icon name="phone" className="h-5 w-5 text-gold-600" />{site.phones[0]}</p>
              </div>
